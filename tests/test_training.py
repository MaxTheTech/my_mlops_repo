...
def test_train_val_split(cfg, mock_data):
    ...
    train_set, val_set = torch.utils.data.random_split(
        mock_data, [train_size, val_size], generator=torch.Generator().manual_seed(cfg.training.seed)
    )
...
def test_metrics_callback():
    ...
    mock_trainer.callback_metrics = {"train_loss": 0.5, "train_acc": 0.95}
...
@pytest.mark.parametrize("batch_size,num_workers", [(32, 0), (64, 2), (128, 4)])
def test_dataloader_creation(mock_data, batch_size, num_workers):
    ...
    dataloader = torch.utils.data.DataLoader(mock_data, batch_size=batch_size, num_workers=num_workers)
